...
def test_api_success(monkeypatch):
    """POST a silent WAV to /callback and assert the expected JSON response.

    External dependencies are monkeypatched to avoid heavy or networked calls:
    - transcribe_with_whisper -> fixed Traditional Chinese string
    - translate_with_openai -> fixed English translation
    - tts_gtts -> fixed bytes to represent MP3
    """
